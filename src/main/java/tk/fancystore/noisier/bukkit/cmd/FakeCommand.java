...
import static tk.fancystore.noisier.bukkit.fake.FakeManager.ALEX;
import static tk.fancystore.noisier.bukkit.fake.FakeManager.STEVE;
...
public class FakeCommand extends Commands {
  ...
  public void perform(CommandSender sender, String label, String[] args) {
    ...
    if (!player.hasPermission("fancycore.cmd.fake") || (label.equalsIgnoreCase("fakel") && !player.hasPermission("fancycore.cmd.fakelist"))) {
    ...
    if (label.equalsIgnoreCase("fake")) {
      ...
      for (int index = 0; index < nicked.size(); index++) {
        sb.append("§c").append(nicked.get(index)).append(" §fé na verdade ").append("§afancycorefakereal:").append(nicked.get(index)).append(index + 1 == nicked.size() ? "" : "\n");
      }
