package tk.fancystore.noisier.bukkit.menus.profile.boosters;
...
import tk.fancystore.noisier.bukkit.menus.profile.MenuBoosters;
...
public class MenuBoostersList<T extends Achievement> extends PagedPlayerMenu {
  ...
  public MenuBoostersList(Profile profile, String name, Booster.BoosterType type) {
    ...
    this.removeSlotsWith(BukkitUtils.deserializeItemStack("INK_SACK:1 : 1 : name>&cVoltar"), 40);
    ...
    for (Booster booster : boosters) {
      ItemStack icon = BukkitUtils.deserializeItemStack(
          "POTION" + (type == Booster.BoosterType.NETWORK ? ":8232" : "") + " : 1 : hide>all : name>&aMultiplicador " + (type == Booster.BoosterType.NETWORK ?
              "Geral" :
              "Pessoal") + " : lore>&fMultiplicador: &6" + booster.getMultiplier() + "x\n&fDuração: &7" + TimeUtils
    ...
    if (items.size() == 0) {
      this.removeSlotsWith(BukkitUtils.deserializeItemStack("WEB : 1 : name>&cVocê não possui nenhum Multiplicador!"), 22);
    }
    this.setItems(items);
    boosters.clear();
    items.clear();
