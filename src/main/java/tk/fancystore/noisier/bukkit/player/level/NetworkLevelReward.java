...
public class NetworkLevelReward {

  private RewardType type;
  private Object[] values;

  public NetworkLevelReward(String reward) {
    if (reward == null) {
      reward = "";
    }
  ...
  public void dispatch(Profile profile) {
    if (this.type == RewardType.COMANDO) {
      ...
      profile.setStats("FancyCoreProfile", profile.getStats("FancyCoreProfile", "cash") + (long) this.values[0], "cash");
    } else if (this.type.name().contains("_COINS")) {
      profile.addCoins("FancyCore" + this.type.name().replace("_COINS", ""), (double) this.values[0]);
