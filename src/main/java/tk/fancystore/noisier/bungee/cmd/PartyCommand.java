...
public class PartyCommand extends Commands {
  ...
  public void perform(CommandSender sender, String[] args) {
    ...
    if (action.equalsIgnoreCase("abrir")) {
      ...
      if (Listeners.getPreferenceOrdinal(action, "pr") == 1) {
        player.sendMessage(TextComponent.fromLegacyText("§c" + Manager.getCurrent(action) + " não está aceitando pedidos de party."));
        return;
      }
