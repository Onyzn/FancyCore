...
public class MySQLDatabase extends Database {
  ...
  public MySQLDatabase(String host, String port, String dbname, String username, String password, boolean mariadb, boolean skipTables) {
    ...
    if (!skipTables) {
      this.update(
          "CREATE TABLE IF NOT EXISTS `FancyCoreNetworkBooster` (`id` VARCHAR(32), `booster` TEXT, `multiplier` DOUBLE, `expires` LONG, PRIMARY KEY(`id`)) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE utf8_bin;");

      DataTable.listTables().forEach(table -> {
        if (!table.getInfo().name().isEmpty() && !table.getInfo().create().isEmpty()) {
  ...
  public void setupBoosters() {
    if (!Manager.BUNGEE) {
      for (String mg : Core.minigames) {
        if (query("SELECT * FROM `FancyCoreNetworkBooster` WHERE `id` = ?", mg) == null) {
          execute("INSERT INTO `FancyCoreNetworkBooster` VALUES (?, ?, ?, ?)", mg, "Noisier_", 1.0, 0L);
        }
      }
    }
  }
  ...
  public void setBooster(String minigame, String booster, double multiplier, long expires) {
    execute("UPDATE `FancyCoreNetworkBooster` SET `booster` = ?, `multiplier` = ?, `expires` = ? WHERE `id` = ?", booster, multiplier, expires, minigame);
  }

  @Override
  public NetworkBooster getBooster(String minigame) {
    try (CachedRowSet rs = query("SELECT * FROM `FancyCoreNetworkBooster` WHERE `id` = ?", minigame)) {
      if (rs != null) {
  ...
  public String getRankAndName(String player) {
    try (CachedRowSet rs = query("SELECT `name`, `role` FROM `FancyCoreProfile` WHERE LOWER(`name`) = ?", player.toLowerCase())) {
      if (rs != null) {
  ...
  @Override
  public int getPreference(String player, String id, int def) {
    int preference = def;

    try (CachedRowSet rs = query("SELECT `preferences` FROM `FancyCoreProfile` WHERE LOWER(`name`) = ?", player.toLowerCase())) {
      if (rs != null) {
        preference = (int) ((JSONObject) new JSONParser().parse(rs.getString("preferences"))).get(id);
      }
  ...
  public Map<String, Map<String, DataContainer>> load(String name) throws ProfileLoadException {
    ...
    for (DataTable table : DataTable.listTables()) {
      if (table.getInfo().name().isEmpty() ||
          table.getInfo().create().isEmpty() ||
          table.getInfo().select().isEmpty() ||
          table.getInfo().insert().isEmpty() ||
          table.getInfo().update().isEmpty()) {
        continue;
      }
  ...
  public void save(String name, Map<String, Map<String, DataContainer>> tableMap) {
    for (DataTable table : DataTable.listTables()) {
      ...
      saveTable(name, table.getInfo().name(), tableMap.get(table.getInfo().name()), true);
    }
  }
  ...
  public void saveSync(String name, Map<String, Map<String, DataContainer>> tableMap) {
    for (DataTable table : DataTable.listTables()) {
      ...
      saveTable(name, table.getInfo().name(), tableMap.get(table.getInfo().name()), false);
    }
  }

  @Override
  public void saveTable(String name, String table, Map<String, DataContainer> rows, boolean async) {
  ...
  public String exists(String name) {
    try {
      return this.query("SELECT `name` FROM `FancyCoreProfile` WHERE LOWER(`name`) = ?", name.toLowerCase()).getString("name");
    } catch (Exception ex) {
      return null;
    }
  }
