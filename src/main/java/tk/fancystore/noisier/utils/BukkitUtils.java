...
import tk.fancystore.noisier.reflection.MinecraftReflection;
import tk.fancystore.noisier.reflection.acessors.ConstructorAccessor;
...
public class BukkitUtils {
   ...
   * <li>enchantments>DAMAGE_ALL:1\nFIRE_ASPECT:1 - Encanta o Item.</li>
   * <li>paint>{@link Color} ou paint>r:g:b - Pinta os Itens: Armadura de Couro e Fogos de
   * Artifício.</li>
   * <li>owner>Notch - Seta o dono de uma cabeça (Recomendado skinvalue>skinvalue}.</li>
   * <li>skinvalue>skinvalue - Seta o valor da Skin através do {@link GameProfile} para cabeças
   * customizadas.</li>
   * <li>pages>Linha1 pagina1\nLinha2 pagina1{skip}Linha1 pagina2\nLinha2 pagina2 - Seta as páginas
   * do livro. (Utilize {pular} para pular para outra página)</li>
   * <li>author>&6Noisier - Seta o autor do livro.</li>
   * <li>title>&6FancyCore - Seta o título do livro.</li>
   * <li>effects>{@link PotionEffectType}:nivel(começa do 0):ticks(20ticks =
   ...
   * </ul>
   *
   * @param item O {@link ItemStack} numa String.
  ...
  public static ItemStack deserializeItemStack(String item) {
    ...
    for (int i = 2; i < split.length; i++) {
      ...
      if (opt.startsWith("name>")) {
        meta.setDisplayName(StringUtils.formatColors(opt.replaceFirst("name>", "")));
      } else if (opt.startsWith("lore>")) {
        for (String lored : opt.replaceFirst("lore>", "").split("\n")) {
          lore.add(StringUtils.formatColors(lored));
        }
      } else if (opt.startsWith("enchantments>")) {
        for (String enchanted : opt.replaceFirst("enchantments>", "").split("\n")) {
          if (enchantment != null) {
        ...
        for (String color : opt.replaceFirst("paint>", "").split("\n")) {
        ...
        skull.setOwner(opt.replaceFirst("owner>", ""));
      } else if (opt.startsWith("skinvalue>") && skull != null) {
        GameProfile gp = new GameProfile(UUID.randomUUID(), null);
        gp.getProperties().put("textures", new Property("textures", opt.replaceFirst("skinvalue>", "")));
        SKULL_META_PROFILE.set(skull, gp);
      } else if (opt.startsWith("pages>") && book != null) {
        book.setPages(opt.replaceFirst("pages>", "").split("\\{skip}"));
      } else if (opt.startsWith("author>") && book != null) {
        book.setAuthor(opt.replaceFirst("author>", ""));
      } else if (opt.startsWith("title>") && book != null) {
        book.setTitle(opt.replaceFirst("title>", ""));
      } else if (opt.startsWith("effects>") && potion != null) {
        for (String pe : opt.replaceFirst("effects>", "").split("\n")) {
        ...
        String[] flags = opt.replaceFirst("hide>", "").split("\n");
  ...
  /**
   * Faz um item "Brilhar" (Encantamento) sem mostrar os encantamentos do item.
   *
